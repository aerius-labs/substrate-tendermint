...
use finality_tendermint::persistent::State;
use log::info;
use parity_scale_codec::{Decode, Encode};

use sc_client_api::backend::AuxStore;
use sp_blockchain::{Error as ClientError, Result as ClientResult};
use sp_finality_tendermint::AuthorityList;
...
pub(crate) fn load_decode<B: AuxStore, T: Decode>(
    ...
    match backend.get_aux(key)? {
        ...
        Some(t) => T::decode(&mut &t[..])
            .map_err(|e| ClientError::Backend(format!("TMNT DB is corrupted: {}", e)))
            .map(Some),
    }
}
    ...
    G: FnOnce() -> ClientResult<AuthorityList>,
{
    ...
    // genesis.
    info!(target: "afp", "Loading TMNT authority set \
