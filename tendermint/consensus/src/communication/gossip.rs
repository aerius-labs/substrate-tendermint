...
fn neighbor_topics<B: BlockT>(peer_round: &PeerRound<NumberFor<B>>) -> Vec<B::Hash> {
    ...
    let mut topics = vec![
        ...
        super::round_topic::<B>(peer_round.round.0, s.0),
    ];
...
pub(super) enum GossipMessage<Block: BlockT> {
    /// Tendermint message with round and set info.
    Vote(VoteMessage<Block>),
    /// Tendermint commit message with round and set info.
    Commit(FullCommitMessage<Block>),
    /// Tendermint global message
    ...
    fn note_catch_up_message_processed(&mut self) {
        match &self.pending_catch_up {
            ...
            state => debug!(target: "afp",
                "Processed catch-up message was noted when the state was: {:?}",
                state,
            ),
        }
    }
...
// Option<GossipMessage<Block>>, Option<Report>
    {
        ...
        let (cost_benefit, topics) = match update_res {
            Ok(round) => (
                ...
                round.map(|round| neighbor_topics::<Block>(round)),
            ),
            ...
            match GossipMessage::<Block>::decode(&mut data) {
                ...
                Ok(GossipMessage::Commit(full)) => {
                    ...
                    peer.round
                        ...
                        == Consider::Accept
                        && Some(&full.message.target_number) == local_round.last_commit_height()
                }
                Ok(GossipMessage::Global(msg)) => {
                    log::debug!(target:"afp", "The global message is not permitted: {:?}", msg);
