...
use sp_finality_tendermint::{AuthorityId, RoundNumber, SetId as SetIdNumber};
use sp_keystore::KeystorePtr;
use sp_runtime::traits::{Block as BlockT, Hash as HashT, Header as HeaderT, NumberFor};

use crate::{
    communication::gossip::VoteMessage, environment::HasVoted, CompactCommit, Error,
    GlobalCommunicationIn, GlobalCommunicationOut, Message, SignedMessage,
};
...
pub mod tendermint_protocol_name {
    ...
    pub(crate) const NAME: &str = "/tendermint/1";
...
/// FIXME: this is a piece of really important code (I guess).
fn incoming_global<B: BlockT>(
    ...
    let process_precommit = {
        ...
        move |msg: FullCommitMessage<B>,
            ...
            if voters.len().get() <= TELEMETRY_VOTERS_LIMIT {
                let preprecommits_signed_by: Vec<String> = msg
                    .message
                    .auth_data
                    .iter()
...
impl<Block: BlockT> Sink<Message<Block>> for OutgoingMessages<Block> {
    ...
    fn start_send(mut self: Pin<&mut Self>, mut msg: Message<Block>) -> Result<(), Self::Error> {
        ...
        if let Some(ref keystore) = self.keystore {
            // QUESTION: how does grandpa deal with it if no new block generated.
...
fn check_compact_precommit<Block: BlockT>(
    ...
    let mut buf = Vec::new();
    for (i, (precommit, (sig, id))) in msg.commits.iter().zip(&msg.auth_data).enumerate() {
