...
use sp_finality_tendermint::{ConsensusLog, ScheduledChange, SetId, TMNT_ENGINE_ID};
use sp_runtime::{
...
use crate::{
    ...
    AuthoritySetChanges, ClientForTendermint, CommandOrError, Error, NewAuthoritySet, VoterCommand,
};

/// A block-import handler for TMNT.
///
...
///
/// When using TMNT, the block import worker should be using this block import
/// object.
pub struct TendermintBlockImport<Backend, Block: BlockT, Client, SC> {
    inner: Arc<Client>,
    select_chain: SC,
...
impl<Backend, Block: BlockT, Client, SC: Clone> Clone
    for TendermintBlockImport<Backend, Block, Client, SC>
{
    fn clone(&self) -> Self {
        TendermintBlockImport {
...
impl<BE, Block: BlockT, Client, SC> JustificationImport<Block>
    for TendermintBlockImport<BE, Block, Client, SC>
where
    NumberFor<Block>: finality_tendermint::BlockNumberOps,
    BE: Backend<Block>,
...
pub fn find_scheduled_change<B: BlockT>(
    ...
    let id = OpaqueDigestItemId::Consensus(&TMNT_ENGINE_ID);
...
impl<BE, Block: BlockT, Client, SC> TendermintBlockImport<BE, Block, Client, SC>
where
    NumberFor<Block>: finality_tendermint::BlockNumberOps,
    BE: Backend<Block>,
    Client: ClientForTendermint<Block, BE>,
    Client::Api: TendermintApi<Block>,
    for<'a> &'a Client: BlockImport<Block, Error = ConsensusError>,
{
    ...
    fn current_set_id(&self, hash: Block::Hash) -> Result<SetId, ConsensusError> {
        ...
        if runtime_version
            .api_version(&<dyn TendermintApi<Block>>::ID)
            .map_or(false, |v| v < 3)
        {
            ...
            for prefix in ["TendermintFinality", "Pbft"] {
    ...
    async fn import_block(
        ...
        match applied_changes {
            AppliedChanges::Forced(new) => {
                ...
                // triggered by this forced change. the new tendermint voter will start at the
        ...
        let tendermint_justification =
            justifications.and_then(|just| just.into_justification(TMNT_ENGINE_ID));

        match tendermint_justification {
            Some(justification) => {
                let import_res = self.import_justification(
                    hash,
                    number,
                    (TMNT_ENGINE_ID, justification),
                    needs_justification,
                    initial_sync,
                );
...
impl<Backend, Block: BlockT, Client, SC> TendermintBlockImport<Backend, Block, Client, SC> {
    pub(crate) fn new(
        inner: Arc<Client>,
        select_chain: SC,
        authority_set: SharedAuthoritySet<Block::Hash, NumberFor<Block>>,
    ...
    NumberFor<Block>: finality_tendermint::BlockNumberOps,
{
    ...
    fn import_justification(
        ...
        if justification.0 != TMNT_ENGINE_ID {
        ...
        let justification = TendermintJustification::decode_and_verify_finalizes(
            &justification.1,
            (hash, number),
        ...
        match result {
            ...
            Err(CommandOrError::Error(e)) => {
                return Err(match e {
                    Error::Tendermint(error) => ConsensusError::ClientImport(error.to_string()),
