...
use authorities::{AuthoritySetChanges, SharedAuthoritySet};
...
use sc_client_api::{
    AuxStore, Backend, BlockchainEvents, CallExecutor, ExecutorProvider, Finalizer, HeaderBackend,
    LockImportRun, StorageProvider,
};
...
impl<Block: BlockT> SharedVoterState<Block> {
    ...
    // TODO: Get the inner `VoterState` instance.
...
pub fn block_import<BE, Block: BlockT, Client, SC>(
    ...
    (
        TendermintBlockImport<BE, Block, Client, SC>,
    ...
    Client: ClientForTendermint<Block, BE> + 'static,
{
    ...
    let (voter_commands_tx, voter_commands_rx) =
        tracing_unbounded("mpsc_tendermint_voter_command", 100_100);
...
impl<B, Block, C, N, S, SC> VoterWork<B, Block, C, N, S, SC>
where
    Block: BlockT,
...
impl<B, Block, C, N, S, SC> Future for VoterWork<B, Block, C, N, S, SC>
where
    Block: BlockT,
    B: Backend<Block> + 'static,
    N: NetworkT<Block> + Sync,
...
pub struct TendermintParams<Block: BlockT, C, N, S, SC> {
...
pub fn run_tendermint_voter<Block: BlockT, BE: 'static, C, N, S, SC>(
    tendermint_params: TendermintParams<Block, C, N, S, SC>,
