    ...
    pub(crate) fn register(registry: &Registry) -> Result<Self, PrometheusError> {
        Ok(Self {
			global_waiting_messages: register(
				Gauge::new(
					"substrate_finality_tendermint_until_imported_waiting_messages_number",
					"Number of finality tendermint messages waiting within the until imported queue.",
				)?,
				registry,
			)?,
...
impl<Block: BlockT> BlockUntilImported<Block> for SignedMessage<Block> {
    ...
    fn wait_completed(self, canon_number: NumberFor<Block>) -> Option<Self::Blocked> {
        ...
        let target_hash = target_hash.expect("we only wait for messages with a target hash; qed");
...

        {
            ...
            match input {
                messages::GlobalMessageIn::Commit(_, ref commit, ..) => {
                    ...
                    let commit_targets = commit
                        .commits
                        .iter()
                        .map(|c| (c.target_height, c.target_hash));

                    for (target_number, target_hash) in commit_targets {
                        let target_hash =
                            target_hash.expect("commit messages always have a target hash; qed");
...
mod tests {
    ...
    impl BlockStatusT<Block> for TestBlockStatus {
        fn block_number(&self, hash: Hash) -> Result<Option<u64>, Error> {
            Ok(self.inner.lock().get(&hash).copied())
        }
    }
        ...
        F: FnOnce(&TestChainState),
    {
        ...
        let (global_tx, global_rx) = tracing_unbounded("test", 100_100);
    ...
    fn blocking_commit_message() {
        let _h1 = make_header(5);
        let _h2 = make_header(6);
        let _h3 = make_header(7);
    ...
    fn blocking_catch_up_message() {
        ...
        // let signed_prevote = |header: &Header| finality_tendermint::SignedPrevote {
        // 	id: UncheckedFrom::unchecked_from([1; 32]),
        // 	signature: UncheckedFrom::unchecked_from([1; 64]),
        // 	prevote: finality_tendermint::Prevote {
        // 		target_hash: header.hash(),
        // 		target_number: *header.number(),
        // 	},
        // };
        //
        // let signed_precommit = |header: &Header| finality_tendermint::SignedPrecommit {
        // 	id: UncheckedFrom::unchecked_from([1; 32]),
        // 	signature: UncheckedFrom::unchecked_from([1; 64]),
        // 	precommit: finality_tendermint::Precommit {
        // 		target_hash: header.hash(),
        // 		target_number: *header.number(),
        // 	},
        // };
        ...
        //
        // let unknown_catch_up = finality_tendermint::CatchUp {
        // 	round_number: 1,
        // 	prevotes,
        // 	precommits,
        // 	base_hash: h1.hash(),
